...
def main(cfg: DictConfig):
    ...
    with open(report_path, 'r') as f_in:
        best_model_params_path = OmegaConf.load(f_in).best_model_params_path
    ...
    report[f'{cfg.experiment_name}/train_roc_auc_score'] = round(float(train_roc_auc_score), 5)
    ...
    report[f'{cfg.experiment_name}/valid_roc_auc_score'] = round(float(valid_roc_auc_score), 5)

    test_roc_auc_score = roc_auc_score(y_test, model.predict_proba(X_test)[:, 1])
    logger.info('test roc_auc_score: %s', test_roc_auc_score)
    report[f'{cfg.experiment_name}/test_roc_auc_score'] = round(float(test_roc_auc_score), 5)

    fig_path_1 = Path(f'{project_dir}/{cfg.paths.report}/{cfg.files.pr_fig_1}')
    ...
    report[f'{cfg.experiment_name}/precision_1'] = round(float(precision_1_score), 5)
    report[f'{cfg.experiment_name}/recall_1'] = round(float(recall_1_score), 5)
    ...
    fig_path_0 = Path(f'{project_dir}/{cfg.paths.report}/{cfg.files.pr_fig_0}')
    ...
    report[f'{cfg.experiment_name}/precision_0'] = round(float(precision_0_score), 5)
    report[f'{cfg.experiment_name}/recall_0'] = round(float(recall_0_score), 5)
    ...
    model_dir_path = Path(f'{project_dir}/{cfg.paths.model}/{cfg.experiment_name}')
